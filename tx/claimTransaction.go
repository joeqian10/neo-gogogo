...
func (tx *ClaimTransaction) Deserialize(br *io.BinaryReader) {
	tx.DeserializeUnsigned(br)
	tx.Transaction.DeserializeWitnesses(br)
}

func (tx *ClaimTransaction) DeserializeUnsigned(br *io.BinaryReader) {
	tx.Transaction.DeserializeUnsigned1(br)
	tx.DeserializeExclusiveData(br)
	tx.Transaction.DeserializeUnsigned2(br)
}

func (tx *ClaimTransaction) DeserializeExclusiveData(br *io.BinaryReader) {
...
func (tx *ClaimTransaction) Serialize(bw *io.BinaryWriter) {
	tx.SerializeUnsigned(bw)
	tx.SerializeWitnesses(bw)
}

func (tx *ClaimTransaction) SerializeUnsigned(bw *io.BinaryWriter) {
	tx.Transaction.SerializeUnsigned1(bw)
	tx.SerializeExclusiveData(bw)
	tx.SerializeUnsigned2(bw)
}

func (tx *ClaimTransaction) SerializeExclusiveData(bw *io.BinaryWriter) {
